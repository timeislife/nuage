    ...
    json.load(configFile, function(config){
        ...
        http.createServer(function (request, response) {
            ...
            if (request.method == 'GET') {
                path.exists(config.directories.application + route.action, function(exists) {
                    console.log(config.directories.application + route.action);
                    if (exists === true) {
                        ...
                        if (action.loginRequired() == true) {
                            if (request.headers.cookie) {
                                ...
                                if (user != null) {
                                    action.execute(request, response, null, user);
                                } else {
                                    action.executeDefault(request, response);
                                }
                            } else {
                                action.executeDefault(request, response);
                            }
                ...
                multipart.handleRequest(request, function(data) {
                    path.exists(config.directories.application + route.action, function(exists) {
                        console.log(config.directories.application + route.action);
                        if (exists === true) {
                            var action = require(config.directories.application + route.action);
